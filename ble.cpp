...
static BLECharacteristic *pAllSensorsChar = nullptr;
...
void ble_set_all_sensors(char id, float valf, int vali) {
    if (pAllSensorsChar == nullptr) {
        Serial.println("BLE error: characteristic not initialized");
        return;
    }
    ...
    if (id == 'E' || id == 'I') {
        snprintf(buffer, sizeof(buffer), "%c!%.2f", id, valf);
    } else {
        snprintf(buffer, sizeof(buffer), "%c!%d", id, vali);
    }

    pAllSensorsChar->setValue((uint8_t*)buffer, strlen(buffer));
